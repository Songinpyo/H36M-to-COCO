...
class H36MDataset(Dataset):
    ...
    def __init__(self,
        ...
        """
        Initializes a new H36MDataset object.
        ...
        self.nof_joints = 17 # number of joints
        self.nof_joints_half_body = 6
        ...
        self.h36m = H36M(self.annotation_path)
        self.imgIds = self.h36m.getImgIds()
        ...
        self.data = []
        # load annotations for each image of h36m
        for imgId in tqdm(self.imgIds):

            ann_ids = self.h36m.getAnnIds(imgIds=imgId, iscrowd=False)

            img = self.h36m.loadImgs(imgId)[0]

            if self.use_gt_bboxes:
                objs = self.h36m.loadAnns(ann_ids)
    ...
    def evaluate_overall_accuracy(self, predictions, bounding_boxes, image_paths, output_dir, rank=0.):
        ...
        self._write_h36m_keypoint_results(oks_nmsed_kpts, res_file)
    ...
    def _write_h36m_keypoint_results(self, keypoints, res_file):
        data_pack = [
            {
                'cat_id': 1,  # 1 == 'person'
                'cls': 'person',
        ...
        results = self._h36m_keypoint_results_one_category_kernel(data_pack[0])
    ...
    def _h36m_keypoint_results_one_category_kernel(self, data_pack):
    ...
    def _do_python_keypoint_eval(self, res_file):
        h36m_dt = self.h36m.loadRes(res_file)
        h36m_eval = H36Meval(self.h36m, h36m_dt, 'keypoints')
        h36m_eval.params.useSegm = None
        h36m_eval.evaluate()
        h36m_eval.accumulate()
        h36m_eval.summarize()
        ...
        for ind, name in enumerate(stats_names):
            info_str.append((name, h36m_eval.stats[ind]))

        return info_str


if __name__ == '__main__':
    h36m = H36MDataset(rotate_prob=0., half_body_prob=0.)
    item = h36m.__getitem__(0)
    print('ok!!')
